...
PROMPT_HTML_JSON = """Task: Edit each value so it reads naturally and fluently for a native speaker while preserving the original meaning, structure, and tone. You are receiving a JSON object mapping {id: text} extracted from HTML text nodes.
...
- Return ONLY a valid JSON object with the SAME KEYS and improved string values. No comments, no code fences, no extra text.
