...
st.title("SEO Humanizer: Anchors + SerfSEO → OpenAI (non-stream) → Ryne AI-score")
st.caption("Промпт зашит в код. Ключи OpenAI и Ryne берутся из .streamlit/secrets.toml")
...
if not OPENAI_API_KEY:
    st.warning("⚠️ В secrets отсутствует OPENAI_API_KEY")
if not RYNE_USER_ID:
    st.warning("⚠️ В secrets отсутствует RYNE_USER_ID")
...
def parse_keywords_ranges(raw: str) -> List[Dict[str, Any]]:
    """
    Варианты ввода:
      1) keyword | min | max | heading:[yes/no]
      2) JSON-массив: [{"keyword":"...", "min":2, "max":5, "heading":true}, ...]
    """
    raw = (raw or "").strip()
    if not raw:
        return []
    ...
    for line in raw.splitlines():
        ...
        kw = cols[0]
        try:
            minv = int(cols[1]); maxv = int(cols[2])
        except Exception:
            continue
        heading = False
        if len(cols) >= 4:
...
def build_inputs_block(
    ...
    headings_line = ", ".join([h for h in (headings_keywords or []) if h]) or "<<list>>"
    whitelist_line = ", ".join([w for w in (whitelist_games or []) if w]) or "<<only titles allowed; do not invent new games>>"
...
def merge_prompt_with_inputs(prompt_template: str, inputs_block: str) -> str:
    """
    Подменяем в шаблоне секцию ## INPUTS целиком на собранный блок, остальное не трогаем.
    """
    pattern = re.compile(r"(## INPUTS)(.*?)(## STYLE TARGETS)", re.DOTALL)
    m = pattern.search(prompt_template)
    if not m:
        # если вдруг не нашли метки — просто конкатенируем
        return prompt_template + "\n\n" + inputs_block
    # заменяем среднюю часть
...
# ============ OpenAI (non-stream) ============
def call_openai_nostream(full_prompt: str) -> str:
    ...
    try:
        resp = client.chat.completions.create(
            ...
            # ВАЖНО: не передаём stream/temperature — некоторые модели их не принимают
        )
...
with left:
    ...
    src = st.text_area("Исходный текст", height=280, placeholder="Вставь оригинальный текст…")

    st.subheader("Anchors")
    st.caption('Формат по строкам: "Текст анкоры" — https://url')
    anchors_raw = st.text_area("Анкоры (по строкам)", height=140,
    ...
    st.caption('Форматы: \n1) keyword | min | max | heading:[yes/no]\n2) JSON-массив объектов.')
    kw_raw = st.text_area("Ключи (диапазоны)", height=160,
...
with right:
    ...
    brand_facts = st.text_area("BRAND_FACTS (hard)", height=90,
                               placeholder='напр.: “Licensed in Curaçao only; do NOT claim AAMS/ADM or other licenses.”')

    st.subheader("KEYWORDS_FOR_HEADINGS")
    headings_list = st.text_input("Список через запятую", "")
...
with colA:
    run_btn   = st.button("🚀 Сгенерировать + 🔎 Ryne AI-score")
with colB:
...
def download_render(text: str):
    if not text:
        st.info("Нет результата для скачивания.")
        return
    if output_fmt == "TXT":
        st.download_button("⬇️ Скачать TXT", data=text.encode("utf-8"),
                           file_name="humanized.txt", mime="text/plain")
    elif output_fmt == "HTML":
        html = to_html(text)
        st.components.v1.html(html, height=420, scrolling=True)
        st.download_button("⬇️ Скачать HTML", data=html.encode("utf-8"),
                           file_name="humanized.html", mime="text/html")
    else:  # DOCX
        if Document is None:
            st.error("python-docx не установлен. Добавь в зависимости.")
            return
        doc = Document()
        for para in (text or "").split("\n"):
            if para.strip():
        ...
        bio = io.BytesIO()
        doc.save(bio)
        st.download_button("⬇️ Скачать DOCX", data=bio.getvalue(),
...
def run_generation_and_score():
    ...
    st.subheader("🧠 OpenAI — генерация (без стрима)")
    with st.spinner("Генерирую текст..."):
        text = call_openai_nostream(full_prompt)

    st.session_state["final_text"] = text or ""

    st.markdown("---")
    st.subheader("📄 Результат")
    st.text_area("Готовый текст", value=text, height=280)
    ...
    if not RYNE_USER_ID:
        ...
        st.stop()
    with st.spinner("Проверяю AI-score..."):
    ...
    if sentences:
        st.write("Разбор по предложениям:")
        for s in sentences:
